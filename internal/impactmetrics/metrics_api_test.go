...
func TestShouldSendWarningWhenIncrementingUndefinedCounter(t *testing.T) {
	...
	api.IncrementCounter("undefined_counter")
...
func TestShouldSendWarningWhenIncrementingCounterByUndefinedCounter(t *testing.T) {
	registry := NewInMemoryMetricRegistry()
	ctx := StaticContext{AppName: "my-app", Environment: "dev"}
	warnings := make(chan error, 3)
	api := NewMetricsAPI(registry, ctx, warnings)

	api.IncrementCounterBy("undefined_counter", 5)
...
func TestShouldSendWarningWhenUpdatingUndefinedGauge(t *testing.T) {
	registry := NewInMemoryMetricRegistry()
	ctx := StaticContext{AppName: "my-app", Environment: "dev"}
	warnings := make(chan error, 3)
	api := NewMetricsAPI(registry, ctx, warnings)

	api.UpdateGauge("undefined_gauge", 42)
...
func TestShouldSendWarningWhenObservingUndefinedHistogram(t *testing.T) {
	registry := NewInMemoryMetricRegistry()
	ctx := StaticContext{AppName: "my-app", Environment: "dev"}
	warnings := make(chan error, 3)
	api := NewMetricsAPI(registry, ctx, warnings)

	api.ObserveHistogram("undefined_histogram", 1.5)
