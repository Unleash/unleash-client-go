...
consists of two components. The first is the repository which runs in a separate Go route and polls the
server to get the latest features toggles. Once the feature toggles are fetched, they are stored by sending
...
creates a set of channels that it passes to both of the above components that it uses for communicating
asynchronously with them. It is ensure that these channels get regularly drained to avoid blocking those
Go routines. There are basically two ways this can be done.
...
that implements 1 or more of the listener interfaces. There are 3 interfaces and you can choose which ones
you should implement:
 - ErrorListener
 - RepositoryListener
 - MetricsListener
If you are only interesting in tracking errors and warning and don't care about any of the other signals,
