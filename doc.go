...
consists of two components. The first is the fetcher which runs in a separate Goroutine and retrieves feature
flag updates from the Unleash server. Once the feature flags are fetched, they are stored by sending
the data to an instance of the Storage interface which is responsible for persisting the data somewhere.
The second component is the metrics component which is responsible for tracking how often features were
queried and whether or not they were enabled. The metrics components also runs in a separate Goroutine and
will occasionally upload the latest metrics to the Unleash server. The client struct creates a set of channels
that it passes to both of the above components and it uses those for communicating asynchronously. It is
important to ensure that these channels get regularly drained to avoid blocking those Goroutines. There
...
If you are only interested in tracking errors and warnings and don't care about any of the other signals,
