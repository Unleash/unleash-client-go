...
func TestGetFeaturesByStrategy(t *testing.T) {
	...
	<-client.Ready()
	// when
	features := client.GetFeaturesByStrategy("MyStrategy")
	// then
	...
	assert.Contains(t, featuresStr, "toto")
	assert.Contains(t, featuresStr, "bar")
	assert.NotContains(t, featuresStr, "foo")
}
