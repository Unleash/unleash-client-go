...
func TestClientWithVariantsAndFeatureDisabled(t *testing.T) {
	assert := assert.New(t)
	defer gock.OffAll()
	...
	assert.Equal(api.GetDefaultVariant(), variant)
	...
	assert.Equal(api.GetDefaultVariant(), variantFromResolver)
